...
hist(colsum2[,2], xlab="Number of steps per day", main="Histogram of Number of Steps Taken Each Day", ylim=c(0,30))
```
...
```
The mean and median for the total number of steps taken per day are `r mean` and `r median`.
...
plot(colmean2[,2], type = "l", xlab="5-minute interval", ylab="Averaged steps taken",main="Time Series Plot of 5-Minute Interval and Averaged Steps")
```
...
```
There are `r nmissing` missing values in this dataset. Assign the mean of that interval's steps to those missing values. Below is the histogram for the imputed data.
...
hist(colsumfinal2[,-1], xlab="Number of steps per day", main="Histogram of Number of Steps Taken Each Day (Imputed Data)", ylim=c(0,30))
```
